...
export default function Job({isLogged, user}) {
    ...
    if(step == 1) return <EmailSent styles={styles} isLogged={isLogged} user={user} email={router.query.email}/>

    if(step == 2) return <NewPasswords styles={styles} isLogged={isLogged} user={user}/>

    if(step == 3) return <PasswordsSuccess styles={styles} isLogged={isLogged} user={user}/>
}
